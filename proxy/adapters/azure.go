...
// AzureOpenAIAdapter proxies requests to Azure OpenAI using the OpenAI API format.
// Azure uses a different endpoint structure and api-key header instead of Bearer auth.
type AzureOpenAIAdapter struct {
	EndpointURL string // e.g. https://{resource}.openai.azure.com
	Client      *http.Client
}

// NewAzureOpenAIAdapter creates a new adapter with the given Azure OpenAI endpoint URL.
func NewAzureOpenAIAdapter(endpointURL string) *AzureOpenAIAdapter {
...
// azureRequestBody is used to extract the model and streaming flag from requests.
...
// azureResponse is the non-streaming response structure (OpenAI format).
...
// azureStreamChunk is a streaming chunk that may contain usage info.
...
// Proxy forwards the request to Azure OpenAI and writes the response back to the client.
// It extracts token usage from both streaming and non-streaming responses.
func (a *AzureOpenAIAdapter) Proxy(w http.ResponseWriter, r *http.Request) (*ProxyResult, error) {
	start := time.Now()
	...
	if reqBody.Stream {
		bodyBytes, err = a.ensureStreamUsage(bodyBytes)
		if err != nil {
			return nil, fmt.Errorf("injecting stream_options: %w", err)
		}
	}

	// Build upstream request to Azure OpenAI.
	// Azure endpoint format: {endpoint}/openai/deployments/{deployment}/chat/completions?api-version=2024-10-21
	// The model field in the request body is used as the deployment name.
	deployment := reqBody.Model
	upstreamURL := fmt.Sprintf("%s/openai/deployments/%s/chat/completions?api-version=2024-10-21",
		strings.TrimRight(a.EndpointURL, "/"), deployment)
	...
	if v := r.Header.Get("api-key"); v != "" {
		upReq.Header.Set("api-key", v)
	}
	// Also support Authorization header passthrough for flexibility.
	if v := r.Header.Get("Authorization"); v != "" {
		upReq.Header.Set("Authorization", v)
	}
	if v := r.Header.Get("Content-Type"); v != "" {
		upReq.Header.Set("Content-Type", v)
	} else {
...
func (a *AzureOpenAIAdapter) ensureStreamUsage(bodyBytes []byte) ([]byte, error) {
...
// proxyNonStreaming handles standard JSON responses from Azure OpenAI.
func (a *AzureOpenAIAdapter) proxyNonStreaming(w http.ResponseWriter, resp *http.Response, result *ProxyResult) error {
	respBody, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("reading upstream response: %w", err)
	}

	// Extract token usage from the response (OpenAI format).
...
// proxyStreaming handles SSE streaming responses from Azure OpenAI.
func (a *AzureOpenAIAdapter) proxyStreaming(w http.ResponseWriter, resp *http.Response, result *ProxyResult) error {
