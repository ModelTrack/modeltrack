...
export default function Reports() {
  ...
  const { data, loading, error, isFirstLoad } = useApi<ExecutiveReport>(url);
  ...
  return (
    ...
    >
      {(report) => {
        const { summary, trends, spend_by_team, spend_by_model, spend_by_feature, daily_spend, optimization_actions, recommendations } = report;
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm text-gray-400">{report.period.label}</p>
              </div>
            </div>
            ...
            <Card>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={chartData}>
                  ...
                  <XAxis dataKey="date" stroke={colors.axis} tick={{ fill: colors.axisLabel, fontSize: 12 }} />
                  <YAxis stroke={colors.axis} tick={{ fill: colors.axisLabel, fontSize: 12 }} tickFormatter={(v) => formatCurrency(v)} />
                  <Tooltip
                  ...
                  />
                  <Area type="monotone" dataKey="cost" stroke={colors.chart.primary} fill="url(#reportCostGradient)" strokeWidth={2} activeDot={{ r: 4, fill: colors.chart.primary, stroke: '#065f46', strokeWidth: 2 }} />
                </AreaChart>
              </ResponsiveContainer>
            </Card>
              ...
              <Card>
                ...
                <ResponsiveContainer width="100%" height={Math.max(200, teamChartData.length * 40)}>
                  <BarChart data={teamChartData} layout="vertical" margin={{ left: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={colors.grid} />
                    <XAxis type="number" stroke={colors.axis} tick={{ fill: colors.axisLabel, fontSize: 12 }} tickFormatter={(v) => formatCurrency(v)} />
                    <YAxis type="category" dataKey="team" stroke={colors.axis} tick={{ fill: colors.axisLabel, fontSize: 12 }} width={100} />
                    <Tooltip
                      cursor={false}
                      contentStyle={chartTooltipStyle.contentStyle}
                      labelStyle={chartTooltipStyle.labelStyle}
              ...
              <Card>
                ...
                <ResponsiveContainer width="100%" height={Math.max(200, modelChartData.length * 40)}>
                  <BarChart data={modelChartData} layout="vertical" margin={{ left: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={colors.grid} />
                    <XAxis type="number" stroke={colors.axis} tick={{ fill: colors.axisLabel, fontSize: 12 }} tickFormatter={(v) => formatCurrency(v)} />
                    <YAxis type="category" dataKey="model" stroke={colors.axis} tick={{ fill: colors.axisLabel, fontSize: 12 }} width={140} />
                    <Tooltip
                      cursor={false}
                      contentStyle={chartTooltipStyle.contentStyle}
                      labelStyle={chartTooltipStyle.labelStyle}
                      ...
                      formatter={(value: number) => [formatCurrency(value), 'Cost']}
                    />
                    <Bar dataKey="total_cost" fill={colors.chart.secondary} radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            </div>
